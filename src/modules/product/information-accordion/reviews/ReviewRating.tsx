...
export const ReviewRating: React.FC<Props> = ({ product, review }) => {
  ...
  const [downVotes, setDownVotes] = React.useState(
    review?.votes_down ? parseInt(review?.votes_down) : 0,
  );
    ...
    <div className="flex gap-3 items-center">
      ...
      <div className="flex gap-2">
        <button
          ...
          className="py-1.5 px-3 transition-all hover:scale-105 hover:shadow text-center border border-gray-300 rounded-md border-gray-400 h-8 text-sm flex items-center gap-1 lg:gap-2"
        >
          <svg
            className="w-4 h-4"
